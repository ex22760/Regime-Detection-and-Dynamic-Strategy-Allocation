# ============================================================
# HYBRID ENSEMBLE REGIME FRAMEWORK — DYNAMIC STRATEGY ALLOCATION
...
#                            RF, LSTM (supervised)
#   Layer 2 — Ensemble:     Weighted arithmetic mean with dynamic
#                            confidence weights (val-period macro-F1)
#   Layer 3 — Allocation:   Probability-weighted strategy blending
#                            (Ang & Bekaert 2004; Nystrup et al. 2017)
#
# Fixes vs original dynamic allocation script:
#   1. VAL_SPLIT_DATE = "2008-01-01" (includes bear days for valid F1)
#   2. Arithmetic mean ensemble (not geometric mean)
#   3. Baseline uses trade-to-BOUNDARY not trade-to-target
#   4. sigma_ann VIX blend before mu_ann (consistent with other scripts)
#   5. mu_ann includes MACRO_WEIGHT (consistent with other scripts)
#   6. base_band uses causal rolling median (no lookahead)
# ============================================================
...
VAL_SPLIT_DATE   = "2008-01-01"   # FIX 1: includes 2008-2009 bear days
...
# FIX 5: mu_ann includes macro_signal
...
# FIX 6: causal rolling median for band
...
band = (
    BAND_MULT
    * np.sqrt(tc)
    * (sigma_ann / sigma_median).clip(0.5, 2.0) * 0.02
)
...
if len(X_val_raw) >= MIN_VAL_OBS:
    ...
    for m, s in val_f1.items():
        print(f"  {m:<6}: {s:.3f}")
    print(f"Val-period bear days: {(y_val_arr == 0).sum()} / {len(y_val_arr)}")
...
# ============================================================
# LAYER 2 — ENSEMBLE: WEIGHTED ARITHMETIC MEAN (FIX 2)
# ============================================================

# ============================================================
# LAYER 2 — ENSEMBLE: COMPARE ARITHMETIC VS GEOMETRIC MEAN
# ============================================================
...
# ── Geometric mean
log_ensemble = (
      w["gmm"]  * np.log(gmm_proba_test  + eps)
    + w["hmm"]  * np.log(hmm_proba_test  + eps)
    + w["rf"]   * np.log(rf_proba_test   + eps)
    + w["lstm"] * np.log(lstm_proba_test + eps)
)
ensemble_geom_raw = np.exp(log_ensemble)
ensemble_geom     = ensemble_geom_raw / ensemble_geom_raw.sum(axis=1, keepdims=True)

# ── Confidence fallback (applied to both)
equal_proba = (
      0.25 * gmm_proba_test
    + 0.25 * hmm_proba_test
    + 0.25 * rf_proba_test
...
for label, ens in [("Arithmetic", ensemble_arith), ("Geometric", ensemble_geom)]:
    conf_tmp  = ens.max(axis=1)
    ens_tmp   = ens.copy()
    ens_tmp[conf_tmp < CONFIDENCE_THRESHOLD] = equal_proba[conf_tmp < CONFIDENCE_THRESHOLD]
    pred_tmp  = ens_tmp.argmax(axis=1)
    bear_pct  = (pred_tmp == 0).mean() * 100
    bear_w    = ens_tmp[:, 0].mean()
    test_idx_tmp = X_test_raw.index
    y_true_tmp   = (y_all.reindex(test_idx_tmp).dropna() == 0).astype(int).values
    print(f"\n  [{label}]")
    print(f"    Bear predicted:     {bear_pct:.1f}%  (true: 1.5%)")
    print(f"    Mean P(bear):       {bear_w:.4f}")
    print(f"    Brier bear:         {brier_score_loss(y_true_tmp, ens_tmp[:len(y_true_tmp), 0]):.4f}")

# ── Choose which to use — set USE_GEOMETRIC = True or False
USE_GEOMETRIC = False   # <-- change this after reviewing the diagnostic above

if USE_GEOMETRIC:
    print("\nUsing: GEOMETRIC MEAN")
    ensemble_proba = ensemble_geom.copy()
else:
    print("\nUsing: ARITHMETIC MEAN")
    ensemble_proba = ensemble_arith.copy()

# Apply confidence fallback to chosen ensemble
...
# ============================================================
# LAYER 3 — DYNAMIC STRATEGY ALLOCATION
#
# Probability-weighted blending (Ang & Bekaert 2004):
#   w* = P(bull)    × w_momentum
#      + P(neutral) × w_vol_target
#      + P(bear)    × w_defensive
# ============================================================

out_test = out.loc[test_idx].copy()

p_bull  = out_test["ensemble_bull"].fillna(1/3).values
p_neut  = out_test["ensemble_neutral"].fillna(1/3).values
p_bear  = out_test["ensemble_bear"].fillna(1/3).values
conf    = out_test["confidence"].fillna(0.5).values
confl   = out_test["conflict"].fillna(0.5).values
w_hjb   = out_test["w_actual"].values

# ── Strategy 1: Momentum (Bull)
mu_ann_test  = mu_ann.reindex(test_idx).values
trend_signal = np.clip(
    mu_ann_test / (sigma_ann.reindex(test_idx).values + 1e-8), -1, 1
)
w_momentum = np.clip(
    w_hjb * (1 + 0.2 * np.maximum(trend_signal, 0)), 0, MAX_WEIGHT
)

# ── Strategy 2: Volatility-targeting (Neutral)
sigma_test = sigma_ann.reindex(test_idx).values
sigma_med  = float(sigma_median.reindex(test_idx).median())
vol_ratio  = np.clip(sigma_test / (sigma_med + 1e-8), 0.5, VOL_NEUTRAL_CAP)
w_neutral  = np.clip(w_hjb / vol_ratio, MIN_WEIGHT, MAX_WEIGHT)

print(f"\nVolatility-targeting neutral strategy:")
print(f"  Median vol (test):     {sigma_med:.3f}")
print(f"  Mean vol_ratio (test): {vol_ratio.mean():.3f}")
print(f"  Mean w_neutral (test): {w_neutral.mean():.3f}  "
      f"(vs mean w_hjb: {w_hjb.mean():.3f})")

# ── Strategy 3: Defensive (Bear)
w_defensive = np.full(len(test_idx), BEAR_FLOOR)
...
# Confidence scaling: shrink toward HJB when ensemble is uncertain
w_blended = conf * w_blended_raw + (1 - conf) * w_hjb
w_blended = np.clip(w_blended, MIN_WEIGHT, MAX_WEIGHT)
w_blended = pd.Series(w_blended, index=test_idx)

out_test["w_momentum"]    = w_momentum
out_test["w_neutral"]     = w_neutral
out_test["w_defensive"]   = w_defensive
out_test["w_blended_raw"] = w_blended_raw

# ── Conflict-adaptive no-trade band
base_band     = BAND_MULT * np.sqrt(tc) * (
    sigma_ann.reindex(test_idx) / sigma_median.reindex(test_idx)
).clip(0.5, 2.0)
conflict_band = base_band * (1 + 1.5 * pd.Series(confl, index=test_idx))
conflict_band = conflict_band.shift(1).fillna(conflict_band.median())

# ── FIX 3: Ensemble — trade to BOUNDARY not target
w_curr = 0.0
equity_ens, weights_ens, trades_count, trades_cost = [], [], 0, 0.0

for date in test_idx:
    tgt     = w_blended.loc[date]
    b       = conflict_band.loc[date]
    base_eq = equity_ens[-1] if equity_ens else 1.0
    ...
    equity_ens.append(base_eq * (1 + port_ret))
    weights_ens.append(w_curr)

out_test["equity_ensemble"] = equity_ens
out_test["w_ensemble"]      = weights_ens

# ── FIX 3: Baseline — trade to BOUNDARY not target
baseline_equity  = []
baseline_eq      = 1.0
w_b_prev         = 0.0
...
print(f"  Ensemble (blended): trades={trades_count:4d}  "
      f"TC drag={trades_cost*100:.2f} bps")
print(f"  Baseline (HJB):     TC drag={base_trades_cost*100:.2f} bps")

# ── Individual strategy isolated backtests
def run_strategy_isolated(w_target_series, label):
    w_s    = 0.0
    eq_s   = []
    tc_s   = 0.0
    band_s = base_band.shift(1).fillna(base_band.median())

    for date in test_idx:
        tgt_s   = float(np.clip(w_target_series.loc[date], MIN_WEIGHT, MAX_WEIGHT))
        b_s     = band_s.loc[date]
        base_eq = eq_s[-1] if eq_s else 1.0

        if abs(tgt_s - w_s) > b_s:
            direction = np.sign(tgt_s - w_s)
            new_w_s   = float(np.clip(w_s + direction * b_s, MIN_WEIGHT, MAX_WEIGHT))
            traded_s  = abs(new_w_s - w_s)
            base_eq   = base_eq * (1 - tc * traded_s)
            tc_s     += tc * traded_s
            w_s       = new_w_s

        asset_ret = ret.reindex(test_idx).loc[date]
        rf_ret    = r_daily.reindex(test_idx).loc[date]
        eq_s.append(base_eq * (1 + w_s * asset_ret + (1 - w_s) * rf_ret))

    equity_s = pd.Series(eq_s, index=test_idx)
    print(f"  {label:<24}: CAGR={segment_cagr(equity_s):.2%}  "
          f"Sharpe={sharpe_ratio(equity_s.pct_change().fillna(0)):.2f}  "
          f"MaxDD={max_drawdown(equity_s):.2%}  "
          f"TC={tc_s*100:.2f}bps")
    return equity_s

print("\n" + "=" * 60)
print("INDIVIDUAL STRATEGY BENCHMARKS (test set)")
print("=" * 60)
eq_momentum  = run_strategy_isolated(pd.Series(w_momentum,  index=test_idx), "Full-time Momentum")
eq_neutral   = run_strategy_isolated(pd.Series(w_neutral,   index=test_idx), "Full-time Vol-Target")
eq_defensive = run_strategy_isolated(pd.Series(w_defensive, index=test_idx), "Full-time Defensive")

out_test["equity_momentum"]  = eq_momentum.values
out_test["equity_neutral"]   = eq_neutral.values
out_test["equity_defensive"] = eq_defensive.values
...
print(f"\nTest-period bear days (true): {(y_true_arr == 0).sum()} / {len(y_true_arr)}")
...
print("=" * 60)
print(f"  {'Strategy':<26}  {'CAGR':>7}  {'Sharpe':>7}  {'MaxDD':>8}")
print(f"  {'-'*26}  {'-'*7}  {'-'*7}  {'-'*8}")
for label, eq in [
    ("Blended Ensemble",     pd.Series(out_test["equity_ensemble"])),
    ("Baseline (HJB)",       pd.Series(out_test["equity_baseline"])),
    ("Full-time Momentum",   pd.Series(out_test["equity_momentum"])),
    ("Full-time Vol-Target", pd.Series(out_test["equity_neutral"])),
    ("Full-time Defensive",  pd.Series(out_test["equity_defensive"])),
]:
    r = eq.pct_change().fillna(0)
    print(f"  {label:<26}  "
          f"{segment_cagr(eq):>7.2%}  "
          f"{sharpe_ratio(r):>7.2f}  "
...
ax.plot(test_idx, out_test["equity_ensemble"],  lw=1.5, color="tab:blue",
        label="Blended Ensemble")
ax.plot(test_idx, out_test["equity_baseline"],  lw=1.2, color="black",
        ls="--", alpha=0.8, label="Baseline (HJB)")
ax.plot(test_idx, out_test["equity_momentum"],  lw=1.0, color="tab:green",
        ls="-.", alpha=0.7, label="Full-time Momentum")
ax.plot(test_idx, out_test["equity_neutral"],   lw=1.0, color="tab:orange",
        ls="-.", alpha=0.7, label="Full-time Vol-Target")
ax.plot(test_idx, out_test["equity_defensive"], lw=1.0, color="tab:red",
        ls=":",  alpha=0.7, label="Full-time Defensive")
ax.set_title("Equity Curves - Blended Ensemble vs Benchmarks")
ax.set_xlabel("Date")
...
# Figure 3 — Strategy weights + rolling return spread (zoomed + full)
...
ax_w.plot(zoom_idx, w_ens_zoom,  lw=1.0, color="tab:blue",
          label="Blended weight")
ax_w.plot(zoom_idx, w_base_zoom, lw=0.9, color="black",
          ls="--", alpha=0.8, label="HJB baseline")
...
ax_w.set_title("Ensemble vs Baseline Weight (Jan 2020 - Jan 2022)")
...
print("\nDone. Saved: fig_hybrid_regime_dashboard.png, fig_ensemble_equity.png,")
print("             fig_weight_comparison.png")
...
bundle = {
    ...
    "gmm_col_order":    gmm_col_order,
    "hmm_col_order":    hmm_col_order,
    "sigma_ann_median": float(sigma_ann.median()),
}

with open(os.path.join(save_dir, "regime_models.pkl"), "wb") as f:
    pickle.dump(bundle, f)

torch.save(lstm_model.state_dict(), os.path.join(save_dir, "lstm_model.pt"))

with open(os.path.join(save_dir, "lstm_input_size.txt"), "w") as f:
    f.write(str(rf_features_train.shape[1]))
